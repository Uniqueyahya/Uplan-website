...
export default function TasksPage() {
      ...
      if (rawTasks) {
        const formatted = rawTasks.map((t: any) => {
          ...
          return {
            id: t.id,
            name: t.name,
            targetValue: t.target_value || (Boolean(t.timer_enabled) ? 5 : 1),
  ...
  const startBleedingTimer = (task: any, customMinutes?: number) => {
    const durationMins = customMinutes || task.targetValue || 5;
    const totalSecs = durationMins * 60;
    setActiveTimerTask({ ...task, targetValue: durationMins });
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-r from-purple-900/40 via-indigo-900/40 to-pink-900/40 border border-purple-500/50 shadow-xl space-y-4 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-purple-300">Active Bleeding Focus Timer ({activeTimerTask.targetValue || 5} min)</span>
                </div>
              ...
              {/* Quick minute adjust buttons */}
              <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-white/10">
                <span className="text-xs text-purple-300 font-semibold mr-1">Switch duration:</span>
                {[2, 3, 5, 10, 15, 25, 30, 45, 60].map((preset) => (
                  <button
                    key={preset}
                    onClick={() => startBleedingTimer(activeTimerTask, preset)}
                    className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all border ${
                      activeTimerTask.targetValue === preset
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              {taskMode === 'timer' && (
                <div className="space-y-2">
                  <label className="block text-xs font-semibold opacity-70 uppercase">Focus Duration (Minutes)</label>
                  
                  {/* Quick Preset Buttons */}
                  <div className="flex flex-wrap gap-1.5">
                    {[2, 3, 5, 10, 15, 20, 25, 30, 45, 60].map((preset) => (
                      <button
                        key={preset}
                        type="button"
                        onClick={() => setTargetDuration(preset)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all border ${
                  ...
                  <input
                    ...
                    onChange={(e) => setTargetDuration(Math.max(1, Number(e.target.value)))}
                    placeholder="Enter any minutes (e.g. 2, 3, 5)"
                    className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500 ${
                      isLight ? 'bg-slate-50 border-slate-300 text-slate-900' : 'bg-[#1c1c1c] border-white/10 text-white'
                    }`}
                  />
                  <span className="text-[11px] opacity-60 block">Set any custom duration: 2 min, 3 min, 5 min, etc.</span>
                </div>
              )}
      ...
      {selectedTask && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className={`w-full max-w-md border rounded-3xl p-6 shadow-2xl space-y-5 ${
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-purple-500">Goal Overview</span>
                <h3 className="text-xl font-bold mt-0.5">{selectedTask.name}</h3>
              </div>
              <button onClick={() => setSelectedTask(null)} className="opacity-60 hover:opacity-100">✕</button>
            </div>

            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-70">Goal Type:</span>
                <span className="font-bold">{selectedTask.timerEnabled ? '⏱️ Timed Focus' : '🌅 Untimed To-Do'}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-70">Target Duration:</span>
                <span className="font-bold text-purple-400">{selectedTask.targetValue || 5} minutes</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-70">Status:</span>
                <span className={`font-bold ${selectedTask.isCompleted ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {selectedTask.isCompleted ? '✅ Completed' : '⏳ In Progress'}
                </span>
              </div>
            </div>

            {selectedTask.timerEnabled && !selectedTask.isCompleted && (
              <div className="space-y-2">
                <label className="block text-xs font-semibold opacity-70 uppercase">Start timer with custom minutes:</label>
                <div className="flex flex-wrap gap-1.5">
                  {[2, 3, 5, 10, 15, 25, 30, 45, 60].map((preset) => (
                    <button
                      key={preset}
                      onClick={() => {
                        startBleedingTimer(selectedTask, preset);
                        setSelectedTask(null);
                      }}
                      className="px-2.5 py-1 rounded-lg text-xs font-bold bg-purple-600/20 hover:bg-purple-600 text-purple-300 hover:text-white border border-purple-500/30 transition-all"
                    >
                      {preset}m
                    </button>
                  ))}
            ...
            <div className="flex items-center gap-3 pt-2">
              {selectedTask.timerEnabled && !selectedTask.isCompleted ? (
                <button
                  type="button"
                  onClick={() => {
                    startBleedingTimer(selectedTask);
                    setSelectedTask(null);
                  }}
                  className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-sm font-bold text-white shadow-md hover:opacity-95 flex items-center justify-center gap-2"
                >
                  <Play className="w-4 h-4 fill-white" /> Start ({selectedTask.targetValue || 5}m)
                </button>
              ) : (
                <button
                  type="button"
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-sm font-bold text-white shadow-md flex items-center justify-center gap-2"
                >
                  <Check className="w-4 h-4" /> {selectedTask.isCompleted ? 'Mark Incomplete' : 'Mark Completed'}
                </button>
              )}
